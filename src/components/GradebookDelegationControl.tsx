...
export function GradebookDelegationControl({ session, beforeChange }: { session: GradebookSession; beforeChange: () => Promise<unknown> }) {
  ...
  const refresh = useCallback(async () => {
    ...
    setDelegations(rows.filter(row => row.gradebook_id === session.id));
  ...
  return <>
    {canManage && <button ref={trigger} type="button" onClick={() => void show()} className="btn border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 !h-10 !px-3"><UserPlus className="h-4 w-4" />มอบหมายครูคนอื่นบันทึกผล</button>}
    {delegations.length > 0 && <span className="max-w-sm text-xs font-semibold text-violet-700">ผู้รับมอบหมาย: {delegations.map(d => d.teacher_name).join(', ')}</span>}
    ...
    {open && createPortal(<div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
      <div ref={dialog} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="delegation-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 text-slate-900 shadow-2xl">
        ...
        {confirm && selected ? <>
          ...
          <div className="flex justify-end gap-3"><button type="button" disabled={busy} className="btn btn-secondary" onClick={() => setOpen(false)}>ปิด</button><button type="button" disabled={!selected || busy} className="btn btn-primary" onClick={() => setConfirm('assign')}>ยืนยัน</button></div>
        </>}
      </div>
    </div>, document.body)}
  </>;
