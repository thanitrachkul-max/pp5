"""Parse foreign language curriculum indicators from school PDF."""
...
OUT = Path(__file__).resolve().parents[1] / "src" / "data" / "curriculum" / "foreign" / "foreignData.json"
...
STRAND_NAMES = {
    1: "การใช้ภาษาพื้นฐาน",
    2: "ทักษะและการประยุกต์ใช้ภาษา",
    3: "การเชื่อมโยงกับกลุ่มสาระการเรียนรู้อื่น",
    4: "การใช้ภาษาในสถานการณ์ต่างๆ",
}

STANDARD_TEXT = {
    "ต 1.1": "พูด ฟัง และอ่านเข้าใจเรื่องราว ข้อความ และคำสั่งโดยใช้คำศัพท์ สำนวน และรูปแบบที่ง่าย มีการใช้เสียงวรรณยุกต์ที่ใกล้เคียงเจ้าของภาษา",
    "ต 1.2": "พูด ฟัง และเขียนสื่อสารในสถานการณ์ที่เกิดขึ้นจริงและจำลอง โดยใช้คำศัพท์ สำนวน และรูปแบบที่เกี่ยวข้อง",
    ...
    "ต 2.2": "เปรียบเทียบและใช้ประโยชน์จากความรู้ ทักษะ และสมรรถนะ ในการสื่อสารภาษาต่างประเทศและภาษาไทย",
    "ต 3.1": "ใช้ภาษาในการเชื่อมโยงความรู้กับกลุ่มสาระการเรียนรู้อื่น และเป็นพื้นฐานในการพัฒนา แสวงหาความรู้ และเปิดโลกทัศน์",
    "ต 4.1": "ใช้ภาษาในสถานการณ์ต่างๆ ทั้งในสถานศึกษา ชุมชน และสังคม",
    "ต 4.2": "ใช้ภาษาเป็นเครื่องมือพื้นฐานในการศึกษาต่อ การประกอบอาชีพ และการแลกเปลี่ยนเรียนรู้กับสังคมโลก",
}
...
def truncate_at_foreign_indicator(text: str, code: str) -> str:
    ...
    for pattern in (FULL_CODE_RE, CODE_RE, ANY_CODE_RE):
        for match in pattern.finditer(text):
            ...
            if matched != code:
                ...
                return text
    for marker in ("มาตรฐาน ต", "กลุ่มที่ ", "สาระที่ ", "ตัวชี้วัดระหว่างทาง", "ตัวชี้วัดปลายทาง"):
...
def resolve_pdf() -> Path:
    for pdf in sorted(PDF_DIR.glob("*.pdf")):
        ...
        doc.close()
        if re.search(r"ต\s*1\.1\s*ป\.1/", full) and "ภาษาต่างประเทศ" in full[:3000]:
            return pdf
    for pdf in sorted(PDF_DIR.glob("*.pdf")):
        if "ภาษาต่างประเทศ" in pdf.name:
            return pdf
...
def append_part(parts: dict[str, list[str]], code: str, text: str) -> None:
    ...
    if GROUP_FOOTER_RE.match(text) or ORPHAN_FRAGMENT_RE.match(text):
        return
    if text.startswith("กลุ่มที่") or text.startswith("มาตรฐาน ต"):
        return
...
def build_indicator_from_parts(code: str, parts: list[str]) -> str | None:
    if not parts:
        return None
    combined = strip_indicator_summary_tail(truncate_at_foreign_indicator(norm(" ".join(parts)), code))
...
def parse_table_rows(all_rows: list[dict[str, str]]) -> dict[str, dict]:
    ...
    for cols in all_rows:
        ...
        if is_noise_continuation(joined) or "มาตรฐาน ต" in cols.get("grade", ""):
            open_grade_midway = None
            open_midway_midway = None
            open_exit = None
            continue
...
def parse_subject_description_pages(doc: fitz.Document) -> list[dict]:
    ...
    for page_index in range(len(doc)):
        ...
        if "คำอธิบายรายวิชา" not in text and "คําอธิบายรายวิชา" not in text:
            continue
        if "ศิลปะ" not in text and "ทัศนศิลป์" not in text and "ดนตรี" not in text and "นาฏศิลป์" not in text:
            continue
